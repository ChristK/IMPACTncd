...
pred.bmi <- function(year, age, sex, qimd, porftvg, lag = cvd.lag) {
  if (is.factor(sex)==F) {
    sex <-  factor(sex, 
                   levels = c(1,2), 
                   ordered = F)
...
bmi.svylm$coefficients["year"] <- -bmi.svylm$coefficients["year"]  
...
# SBP 
pred.sbp <- function(year, age, sex, qimd, bmival, cigst1, porftvg, lag = cvd.lag) {
  if (is.factor(sex)==F) {
    sex <-  factor(sex, 
                   levels = c(1,2), 
                   ordered = F)
...
sbp.svylm$coefficients["(Intercept)"] <- sbp.svylm$coefficients["(Intercept)"]  - 10
...
pred.chol <- function(year, age, sex, qimd, bmival, porftvg, lag = cvd.lag) {
  if (is.factor(sex)==F) {
    sex <-  factor(sex, 
                   levels = c(1,2), 
                   ordered = F)
...
plot(pred.chol  (0:4, 50:54, 1, 1, 30, 5), ylim=c(3,6), xlim=c(0,30))
points(pred.chol(0:50, 50, 1, 3, 30, 5), col="blue")
points(pred.chol(0:50, 50, 1, 5, 30, 5), col="red")

chol.svylm$coefficients["(Intercept)"] <- chol.svylm$coefficients["(Intercept)"] - 0.5
points(x = 9, pred.chol(9, 59, 1, 1, 30, 5))
