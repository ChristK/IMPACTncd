#cmpfile("./Scenarios/smoking ban.R")
...
if (i > (intervention.year - 2011 + cvd.lag) & i <= (intervention.year - 2011 + diffusion.time + cvd.lag)) {
  policy.effect.cigdyal <- 0.5 * (i - (intervention.year - 2011 + cvd.lag))/diffusion.time # effect of policy on cigarette consumption (50% reduction)
}
...
if (i > (intervention.year - 2011 + diffusion.time + cvd.lag)) {
  policy.effect.cigdyal <- 0.5 # effect of policy on cigarette consumption (50% reduction)
}
