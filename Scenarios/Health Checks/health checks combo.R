#cmpfile("./Scenarios/health checks combo.R")
...
if (i == (init.year - 2011)) {
  ...
  post.ageing.scenario.fn <- cmpfun(function(i, env = my.env) {
    ...
    if (i >  (intervention.year - 2011) &
        i <= (intervention.year - 2011 + 5)) {
      POP[cigst1 == "4",
          cigst1.temp :=
            ifelse(dice(.N) < smoking.decr + (i - (intervention.year - 2011 + cvd.lag)) /
                     50,
                   T, F)]
      POP[cigst1.temp == T,
          `:=`(
    ...
    if (i > intervention.year - 2011) {
      ...
      POP[cigst1 == "4" &
            high.risk2 == 1 &
            rbinom(.N, 1, 0.2) * rbinom(.N, 1, 0.1) == 1,
          `:=`(
            cigst1 = "3",
            endsmoke.curr = 0,
            numsmok.curr = cigdyal.curr
          )] # first binom is the healthcheck
      #every 5 years the 2nd is the success rate of smoking cessation
      
      
      output <- vector("list", 2)
      
      if (exists("highrisk.rds"))
        output[[1]] <- highrisk.rds
    ...
    if (i > intervention.year - 2011 + cvd.lag) {
      ...
      POP[high.risk2 == 1 &
            between(qrisk2, 0.1, 0.2) & omsysval.cvdlag >= 135,
          `:=` (
      ...
      POP[high.risk2 == 1 &
            between(qrisk2, 0.1, 0.2) & cholval.cvdlag >= 5,
          `:=` (
      ...
      POP[high.risk2 == 1 &
            bmival.cvdlag > 50 & rbinom(.N, 1, 0.2) == 1,
          bmival.cvdlag := 30]
...
if (i > intervention.year - 2011 + cvd.lag &
    i <= intervention.year - 2011 + cvd.lag + 5) {
  ...
  sbp.svylm$coefficients["(Intercept)"] <-
    sbp.svylm$coefficients["(Intercept)"] - sbp.decr / 5 # 1.3 in 5 years (salt mandatory reformulation)
  ...
  FV.intervention <- 1L
  FV.intervention.success <-
    (i - cvd.lag) * fv.decr / 5 # 0.5 in 5 years
}
