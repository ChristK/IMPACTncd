...
SynthPOP <- function(i = 1) {
  ...
  # Calculate packyears
  HSE2011 = copy(HSE2011original)
  HSE2011[startsmk==97, startsmk := NA]
  ...
  #Sanity check
  SPOP2011[as.numeric(as.character(age)) < 16, packyears := 0]
  SPOP2011[as.numeric(as.character(age)) > 15 & packyears>(as.numeric(as.character(age)) - 15) * 3, 
  ...
  HSE2011[, packyearsCat := cut(packyears, breaks = breaks, 
                                include.lowest = T, 
                                ordered_result = TRUE)]
  SPOP2011[, packyearsCat := cut(packyears, breaks = breaks, 
                                 include.lowest = T, 
                                 ordered_result = TRUE)]
...
# Finalise SPOP2011 -------------------------------------------------------
  ...
  SPOP2011[, a30to06m := as.integer(as.character(a30to06m))]
  SPOP2011[, cigdyalCat := as.numeric(as.character(cigdyalCat))]
  SPOP2011[, endsmoke := as.integer(as.character(endsmoke))]
