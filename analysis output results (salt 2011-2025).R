...
HSE.ts[, salt.intersalt := Na24 * 58.5/1000]

HSE.ts[between(age, 30, 84), .("mean salt (g/d)" = mean(salt.intersalt, na.rm = T)), keyby = age
       ][, plot(age, `mean salt (g/d)`)]
...
load("~/pCloudDrive/Datasets/Health Survey for England/2014/hse2014ai.RData")
setDT(HSE2014original)
HSE2014original[age90>15 & sex== "1",
       Na24 := Na24.men.det(.N, sodium, creatin, potass, bmival, age90)]
HSE2014original[age90>15 & sex== "2",
       Na24 := Na24.women.det(.N, sodium, creatin, potass, bmival, age90)]
HSE2014original[, salt.intersalt := Na24 * 58.5/1000]

HSE.ts[year<0, glm(salt.intersalt~factor(qimd, ordered = F) + age + sex, weights = wt.urine)]
HSE2014original[, glm(salt.intersalt~factor(qimd) + age90 + sex, weights = wt.urine)]
# gg <-
#   ggplot(tt[between(year, 2006, 2025), ],
#          aes(x = year, y = salt, 
#              col = scenario, linetype = rev(scenario), fill = scenario)) +
...
# Burden estimation 2011 - 202x --------------------------
ext <- function(hor, strata = c("qimd", NULL)) {
  pr <- c(0.5, 0.25, 0.75, 0.1, 0.9)
  str <- c("mc", strata)
  res1 <- 
    cpp[between(year, 2011 + cvd_lagtimes, hor + cvd_lagtimes), .(
  ...
  res3 <- 
    cpp[between(year, 2011 + cancer_lagtimes, hor + cancer_lagtimes), .(gca.cases = round(sum(`gca.cases_Responsibility Deal` - 
                                                                                                `gca.cases_FSA Led`)),
  ...
  res4 <- 
    cpp[between(year, 2011 + cancer_lagtimes, hor + cancer_lagtimes), 
...
fwrite(ext(2025, NULL), paste0(save_dir, "results_20112025.csv"))
# fwrite(ext(2025, NULL), paste0(save_dir, "results_2025.csv"))

fwrite(ext(2025, "qimd"), paste0(save_dir, "results_20112025_qimd.csv"))
# fwrite(ext(2025, "qimd"), paste0(save_dir, "results_2025_qimd.csv"))

hor <- 2025
str <- c("mc", "qimd")
t1 <- cpp[between(year, 2011 + cvd_lagtimes, hor + cvd_lagtimes) & qimd == 1, .(
  cvd.cases = sum(`cvd.cases_Responsibility Deal` -
                    `cvd.cases_FSA Led`)
),
by = str]

t5 <- cpp[between(year, 2011 + cvd_lagtimes, hor + cvd_lagtimes) & qimd == 5, .(
  cvd.cases = sum(`cvd.cases_Responsibility Deal` -
                    `cvd.cases_FSA Led`)
),
by = str]
...
t1 <- cpp[between(year, 2011 + cancer_lagtimes, hor + cancer_lagtimes) & qimd == 1, .(
  gca.cases = sum(`gca.cases_Responsibility Deal` -
                    `gca.cases_FSA Led`)
),
by = str]

t5 <- cpp[between(year, 2011 + cancer_lagtimes, hor + cancer_lagtimes) & qimd == 5, .(
  gca.cases = sum(`gca.cases_Responsibility Deal` -
                    `gca.cases_FSA Led`)
),
by = str]
...
hor <- 2025
chd.cost <- chd.burden[group == "SAQ" & between(year, 2011 + cvd_lagtimes, hor + cvd_lagtimes),
                       .(chd.incidence,
                         chd.prevalence,
                         chd.mortality, pop), 
                       by = .(year, scenario, sex, agegroup, qimd, mc)]
...
stroke.cost <- stroke.burden[group == "SAQ" & between(year, 2011 + cvd_lagtimes, hor + cvd_lagtimes), 
                             .(stroke.incidence,
                               stroke.prevalence,
                               stroke.mortality), 
                             by = .(year, scenario, sex, agegroup, qimd, mc)]
...
# 50% 25% 75% 
# 61  10 110 
c16.cost <- c16.burden[group == "SAQ" & between(year, 2011 + cancer_lagtimes, hor + cancer_lagtimes),
                       .(c16.incidence,
                         c16.prevalence,
                         c16.mortality, pop), 
                       by = .(year, scenario, sex, agegroup, qimd, mc)]
...
# 50% 25% 75% 
# 71  16 130 
fwrite(res, paste0(save_dir, "costs_20112025.csv"))
