#cmpfile("./Scenarios/high risk max2.R")
...
if (i == (init.year - 2011)) {
  ...
  post.ageing.scenario.fn <- cmpfun(function(i, env = my.env) {
    ...
    if (i > intervention.year - 2011) {
      ...
      diffusion <- sample_frac(POP[ high.risk == T, .(id)], 0.2)
      POP[id == diffusion,
          `:=` (high.risk6 = T,
                high.risk2 = T)]
      }
      ...
      if (i == intervention.year - 2011 + 2) {
        diffusion <- sample_frac(POP[ high.risk == T, .(id)], 0.4)
        POP[id == diffusion,
            `:=` (high.risk5 = T,
                  high.risk2 = T)]
      }
      ...
      if (i == intervention.year - 2011 + 3) {
        diffusion <- sample_frac(POP[ high.risk == T, .(id)], 0.6)
        POP[id == diffusion,
            `:=` (high.risk4 = T,
                  high.risk2 = T)]
      }
      ...
      if (i == intervention.year - 2011 + 4) {
        diffusion <- sample_frac(POP[ high.risk == T, .(id)], 0.7)
        POP[id == diffusion,
            `:=` (high.risk3 = T,
                  high.risk2 = T)]
      }
      ...
      if (i > intervention.year - 2011 + 4) {
        diffusion <- sample_frac(POP[ high.risk == T, .(id)], 0.8)
        POP[id == diffusion,
            `:=` (high.risk2 = T)]
      }
