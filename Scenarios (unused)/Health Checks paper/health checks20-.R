...
#           high.risk := T]
      ...
      if (i == intervention.year - 2011 + 1) {
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "1", .(id)], 0.03)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk6 = T)]
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "2", .(id)], 0.04)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk6 = T)]
        ...
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk6 = T)]
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "4", .(id)], 0.06)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk6 = T)]
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "5", .(id)], 0.07)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk6 = T)]
      }
      ...
      if (i == intervention.year - 2011 + 2) {
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "1", .(id)], 0.08)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk5 = T)]
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "2", .(id)], 0.09)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk5 = T)]
        ...
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk5 = T)]
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "4", .(id)], 0.11)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk5 = T)]
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "5", .(id)], 0.12)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk5 = T)]
      }
      ...
      if (i == intervention.year - 2011 + 3) {
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "1", .(id)], 0.11)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk4 = T)]
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "2", .(id)], 0.13)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk4 = T)]
        ...
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk4 = T)]
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "4", .(id)], 0.17)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk4 = T)]
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "5", .(id)], 0.19)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk4 = T)]
      }
      ...
      if (i == intervention.year - 2011 + 4) {
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "1", .(id)], 0.13)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk3 = T)]
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "2", .(id)], 0.15)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk3 = T)]
        ...
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk3 = T)]
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "4", .(id)], 0.2)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk3 = T)]
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "5", .(id)], 0.23)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T,
                  high.risk3 = T)]
      }
      ...
      if (i >= intervention.year - 2011 + 5) {
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "1", .(id)], 0.15)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T)]
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "2", .(id)], 0.165) # 0.175
        POP[id %in% diffusion,
            `:=` (high.risk2 = T)]
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "3", .(id)], 0.2)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T)]
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "4", .(id)], 0.225)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T)]
        diffusion <- sample_frac(POP[ high.risk == T & qimd == "5", .(id)], 0.25)
        POP[id %in% diffusion,
            `:=` (high.risk2 = T)]
      }
