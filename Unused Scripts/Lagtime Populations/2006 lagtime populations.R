...
pop <- 50965200  # Mid year population
...
HSE <- data.table(HSE, key="age")
HSE =copy(HSE[age > 15,])

load(file="./Datasets/Health Survey for England/2006/hse06ah.RData")
HSEhh <- data.table(HSE2006hh, key="age")
...
setnames(HSE, c("hhsize", "imd2004", "newsha"), c("hsize", "qimd", "sha"))
...
breaks <- c(0, seq(0.1, 99.1, 10), max(HSE$expsm, na.rm = T))
