...
# User input
year <- 2007
prop <- 0.05  # Set propotion (prop) of the populations to be synthesized (set 1 for total population)
pop <- 51381100  # Mid year population
...
HSE <- data.table(HSE2007original, key="age")
rm(HSE2007original)
HSE =copy(HSE[age > 15 & samptype == 1,])

load(file="./Datasets/Health Survey for England/2007/hse07ah.RData")
HSEhh <- data.table(HSE2007hh, key="age")
HSEhh =copy(HSEhh[age > 15,])
rm(HSE2007hh)
...
setnames(HSE, c("hhsized", "imd2007", "newsha"), c("hsize", "qimd", "sha"))
...
basic <- c("agegroup", "sex", "hpnssec8", "eqv5", "cigst1" )
...
breaks <- c(0, seq(0.1, 99.1, 10), HSE[,max(expsm, na.rm = T)])
HSE[, expsmokCat := cut(expsm, breaks = breaks,
                        labels = c(0:10),
                        include.lowest = T, 
                        ordered_result = T)]
basic <- c("agegroup", "sex", "hpnssec8", "eqv5", "cigst1" )
...
SPOP <- simCategorical(HSE, SPOP, 
                       ...
                       method = "multinom", 
                       MaxNWts = 6000)
if (SPOP[is.na(redmeat) == T, .N] > 0) {
    SPOP <- SPOP[is.na(redmeat) == F,] # Remove NA
}
...
HSE[, bmivalCat := cut(bmival, breaks = breaks,
                       include.lowest = T, 
                       ordered_result = T)]
basic <- c("agegroup", "sex", "hpnssec8", "eqv5", "bmivalCat")
