...
if (Sys.info()[1] == "Linux") {
  if (system("whoami", T) == "mdxasck2") {
    ...
    setwd(paste("/home/", system("whoami", T), "/Dropbox/PhD/Models/IMPACTncd/", 
      sep = "", collapse = ""))
  }
} else {
...
foreach(iterations = 1:it, .inorder = F, .verbose = T, .packages = c("data.table", 
  "dplyr", "randtoolbox", "stringr", "compiler", "mc2d", "quantreg", 
  "nnet"), .export = ls(), .noexport = c("scenarios.list", "time.mark")) %dorng% 
  {
